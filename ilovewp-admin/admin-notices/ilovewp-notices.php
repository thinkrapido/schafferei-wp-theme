...
class city_hall_notice {
	...
	private function is_dismiss_notice() {
		return apply_filters( 'city_hall_' . $this->name . '_notice_dismiss', true );
	}
	...
	public function hide_notices() {
		...
		if ( isset( $_GET['city-hall-hide-notice'] ) && isset( $_GET['_city_hall_notice_nonce'] ) ) {
			
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_city_hall_notice_nonce'] ), 'city_hall_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'city-hall' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'city-hall' ) );
			}

			if ( $_GET['city-hall-hide-notice'] ) {
				
				$hide_notice_id = sanitize_text_field( wp_unslash( $_GET['city-hall-hide-notice'] ) );
				...
				if ( isset($run_update) ) {
					update_option( 'city_hall_admin_notices', $theme_admin_notices );
				}
